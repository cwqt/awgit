...
const getAllDays: VercelFunction = async (req, fs): Promise<Array<Omit<IDay, 'commits'>>> => {
  ...
  let endDate = end ? new Date(end) : addDay(startDate, -(DAY_LIMIT - 1));

  const dayDifference = Math.floor((endDate.getTime() - startDate.getTime()) / (1000 * 3600 * 24));
  if(Math.abs(dayDifference) > DAY_LIMIT) throw Error(`Must be less-than-or-equal to 100 days between dates`)
